...
function Score({score}) {
    return(
        <div id="score">
            <p id="score-text">Score: {score[0]}/{score[1]}</p>
        </div>
    )
}
...
function ImageExpanded({images, text, addedClass, id, title, expandedText, score, setScore, navs, quizMaterial, setQuizReset, fullQuiz}) {
    const [fullscreen, setFullscreen] = useState(false);
    const [styling, setStyling] = useState([{}, {}]);
    const [isOpen, setIsOpen] = useState("closed");
    const [curTitle, setCurTitle] = useState(0);
    const [curDescription, setCurDescription] = useState(0);
    ...
    const setQuizMaterial = (param) => {
        let trueId = id - 1;
        setQuizReset([...fullQuiz.slice(0, trueId), param, ...fullQuiz.slice(trueId + 1)]);
    }
        ...
        return(
            <div style={{"height" : "0", "width": "0"}}>
                <div className={"image-expanded-container-expanded"} id={"image-expanded-container-expanded" + id}>
                    ...
                    <h1 className="image-title">{title[curTitle]}</h1>
                    <ImageExpandedDescription isQuiz={isQuiz} curDescription={expandedText[curDescription]} id={id} quizMaterial={quizMaterial} setQuizMaterial={setQuizMaterial} score={score}  setScore={setScore}/>
                </div>
            </div>
        );
    } else {
...
function ImageContainer({images, text, id, title, expandedText, score, setScore, navs, quizReset, setQuizReset, fullQuiz}) {
    return(
        <div className="image-container" id={"image-container-" + id}>
            <Images images={images} id={id}/>
            <ImageExpanded images={images} text={text} addedClass={id % 2 === 1 ? "odd" : "even"} id={id} title={title} expandedText={expandedText} score={score}  setScore={setScore} navs={navs} quizMaterial={quizReset} setQuizReset={setQuizReset} fullQuiz={fullQuiz}/>
        </div>
    );
}

function Options({images, text, title, expandedText, score, setScore, quizReset, setQuizReset}) {
    return(
        <div id="options">
            <div id="line"></div>
            <ImageContainer images={images[0]} text={text[0]} id={1} title={title[0]} expandedText={expandedText[0]} score={score} setScore={setScore} navs={title[5]} quizReset={quizReset[0]} setQuizReset={setQuizReset} fullQuiz={quizReset}/>
            <ImageContainer images={images[1]} text={text[1]} id={2} title={title[1]} expandedText={expandedText[1]} score={score} setScore={setScore} navs={title[5]} quizReset={quizReset[1]} setQuizReset={setQuizReset} fullQuiz={quizReset}/>
            <ImageContainer images={images[2]} text={text[2]} id={3} title={title[2]} expandedText={expandedText[2]} score={score} setScore={setScore} navs={title[5]} quizReset={quizReset[2]} setQuizReset={setQuizReset} fullQuiz={quizReset}/>
            <ImageContainer images={images[3]} text={text[3]} id={4} title={title[3]} expandedText={expandedText[3]} score={score} setScore={setScore} navs={title[5]} quizReset={quizReset[3]} setQuizReset={setQuizReset} fullQuiz={quizReset}/>
            <ImageContainer images={images[4]} text={text[4]} id={5} title={title[4]} expandedText={expandedText[4]} score={score} setScore={setScore} navs={title[5]} quizReset={quizReset[4]} setQuizReset={setQuizReset} fullQuiz={quizReset}/>
        </div>
    );
}

function Dropdown({setScore, setImages, setText, setTitle, setExpandedText, setQuizReset, setEntireTitle}) {

    const resetModuleI = () => {
        setScore(["-", "-"]);
        setImages(["https://upload.wikimedia.org/wikipedia/commons/thumb/0/02/Sea_Otter_%28Enhydra_lutris%29_%2825169790524%29_crop.jpg/800px-Sea_Otter_%28Enhydra_lutris%29_%2825169790524%29_crop.jpg"
    ...
    "https://optimise2.assets-servd.host/maniacal-finch/production/animals/amur-tiger-01-01.jpg?w=1200&auto=compress%2Cformat&fit=crop&dm=1658935145&s=1b96c26544a1ee414f976c17b18f2811"]);
        setText(["Otters: aquatic species key to our ecosystem, expand to learn more!", "Kelp forests: vast marine forests that are home to many species, expand to learn more!", "Coral reefs: giant thriving ecosystems with vibrant colors, expand to learn more!", "Elephants: giant land mammals that create habitats and ecosystems, expand to learn more!", "Tigers: apex predators and relatives to housecats, expand to learn more!"]);
        setTitle([["What are otters?", "Where do otters live?", "Quiz!"], 
        ...
        setExpandedText([["Otters are a member of the weasel family, with all of their species being aquatic or semi-aquatic. They are both strong and fast swimmers, being very mobile in the water! Being a keystone species and a top predator, they control and eat populations of urchins to keep a balanced ecosystem!",
        ...
        setQuizReset([[["Urchin", "1"], //what is the current id of the radio button selected
        false, //is the entire quiz finished / disabled
        [true, false, false ,false], //what should the quiz reload as defaultChecked next time?
        ["transparent", "transparent", "transparent", "transparent"], //what's the colors of the answer choices?
        ["", "hidden", "0", "", true] //the message that displays when quiz is checked (correct or incorrect)
        ], [["Coastlines", "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [["Calcium Carbonate", "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [["Africa and Asia", "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [["1 and a half years", "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]]]
        );
        setEntireTitle("Module I: Biodiversity");
    }
    ...
    const resetModuleII = () => {
        ...
        setQuizReset([[["Because it is energy from the Sun", "1"], //what is the current id of the radio button selected
        false, //is the entire quiz finished / disabled
        [true, false, false ,false], //what should the quiz reload as defaultChecked next time?
        ["transparent", "transparent", "transparent", "transparent"], //what's the colors of the answer choices?
        ["", "hidden", "0", "", true] //the message that displays when quiz is checked (correct or incorrect)
        ], [["Lignite", "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [["~40%", "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [["pressurizing sand, water, and chemicals underground to extract natural gas", "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [["fracking", "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]]]
    );
    setEntireTitle("Module II: Non-renewable Resources");
    }

    return(
        <div id="switch-modules">
            <div className="modules" onClick={() => resetModuleI()}>
                <p className="module-text">Module I</p>
            </div>
            <div className="modules" onClick={() => resetModuleII()}>
                <p className="module-text">Module II</p>
            </div>
        </div>
...
"https://optimise2.assets-servd.host/maniacal-finch/production/animals/amur-tiger-01-01.jpg?w=1200&auto=compress%2Cformat&fit=crop&dm=1658935145&s=1b96c26544a1ee414f976c17b18f2811"]);
    ...
    const[quizReset, setQuizReset] = useState([[[expandedText[0][2][1], "1"], //what is the current id of the radio button selected
        false, //is the entire quiz finished / disabled
        [true, false, false ,false], //what should the quiz reload as defaultChecked next time?
        ["transparent", "transparent", "transparent", "transparent"], //what's the colors of the answer choices?
        ["", "hidden", "0", "", true] //the message that displays when quiz is checked (correct or incorrect)
        ], [[expandedText[1][2][1], "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [[expandedText[2][2][1], "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [[expandedText[3][2][1], "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]], 
        [[expandedText[4][2][1], "1"], false, [true, false, false ,false], ["transparent", "transparent", "transparent", "transparent"], ["", "hidden", "0", "", true]]]
    );
    const [entireTitle, setEntireTitle] = useState("Module I: Biodiversity");

    return(
        <div id="app">
            <h1 id="title">APES Material</h1>
            <h2 id="subtitle">{entireTitle}</h2>
            <Score score={score} />
            <Options images={images} text={text} title={title} expandedText={expandedText} score={score} setScore={setScore} quizReset={quizReset} setQuizReset={setQuizReset}/>
            <Dropdown setScore={setScore} setImages={setImages} setText={setText} setTitle={setTitle} setExpandedText={setExpandedText} setQuizReset={setQuizReset} setEntireTitle={setEntireTitle}/>
        </div>
    );
}
